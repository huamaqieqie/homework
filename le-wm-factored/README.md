...
# Factored LeWorldModel (Factored LeWM)

This directory is an independent Factored Latent implementation derived from
LeWM and its residual-predictor variant. The 192-dimensional latent is split
into static and dynamic branches. Only the dynamic branch is passed to the
action-conditioned predictor:

```text
z_t = [s_t, d_t]
s_hat_{t+1} = s_t
d_hat_{t+1} = d_t + Delta(d_t, a_t)
z_hat_{t+1} = [s_hat_{t+1}, d_hat_{t+1}]
```

The default split is 96 static dimensions and 96 dynamic dimensions. See
[`docs/factored_lewm.md`](docs/factored_lewm.md) for the loss, configuration,
metrics, latent export fields, and server commands.
